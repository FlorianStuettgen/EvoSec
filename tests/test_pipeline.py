...
class PipelineTests(unittest.TestCase):
    def test_all_reference_scenarios_pass(self) -> None:
        expected = {
            "network-scan": 1,
        ...
        for name, count in expected.items():
            with self.subTest(name=name):
                ...
                self.assertEqual(
                    [entry.stage for entry in result.ledger.entries], ["load", "compile", "index", "evaluate", "verify"]
                )
    ...
    def test_max_event_limit_is_enforced(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "max_events"):
            # ValidationError is a ValueError subtype by contract.
